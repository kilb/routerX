...
class D4a_TokenizerFingerprint(BaseDetector):
    ...
    def _compare_tokens(
        ...
        # When the claimed model runs through a proxy that translates
        # between API formats (e.g. Anthropic model via OpenAI-compatible
        # endpoint), logprobs may be generated by the proxy layer rather than
        # the actual model, causing a tokenizer mismatch that is NOT fraud.
        # Degrade to INCONCLUSIVE for non-OpenAI models on OpenAI-format
        # proxies, since we can't distinguish proxy artifact from substitution.
        model_lower = claimed_model.lower()
        # Only treat as proxy artifact when the model name clearly belongs
        # to a different provider family (e.g. "claude" on an OpenAI-format
        # endpoint). For OpenAI model names (gpt-*, o1-*, o3-*) or unknown
        # names, a tokenizer mismatch is suspicious even through a proxy.
        non_openai_model = any(
            k in model_lower for k in ("claude", "gemini", "llama", "qwen", "mistral")
        )
        is_proxy_artifact = (
            non_openai_model
            and self.config.claimed_provider in (ProviderType.ANTHROPIC, ProviderType.GEMINI, ProviderType.ANY)
        )
        if is_proxy_artifact:
            return self._inconclusive(
                f"tokenizer mismatch (may be proxy translation artifact for "
                f"{claimed_model!r})"
            )
