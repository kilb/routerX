...
class D60_LatencyFingerprint(BaseDetector):
    ...
    def judge(self, responses: list[ProbeResponse]) -> DetectorResult:
        ...
        if band is None:
            return self._pass({"note": f"no latency band for model {self.config.claimed_model!r}"})
        ...
        if local_tokens < 20:
            return self._pass({"note": f"only {local_tokens} tokens -- too few for reliable tps measurement"})
        ...
        if max_delta < _MIN_EDGE_DELTA:
            return self._pass({"note": f"latency near band edge (max delta {max_delta:.2f})"})
        return self._fail(
