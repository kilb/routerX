...
> **误报过滤：** 已应用 22 条已修复误报规则，从 116 项原始 FAIL 中排除 38 项，剩余 78 项真实检测结果
...
- **D22 JSON Schema 不支持**（2 个供应商、4 个模型）— 影响 openai（gpt-5/o4-mini 系列）和 amazon（nova-pro）。
...
**严重级别：** P0 — 协议违规  
**受影响模型（4 个）：** openai/gpt-5、openai/o4-mini、openai/o4-mini-high、amazon/nova-pro-v1
...
**严重级别：** P0 — 供应链安全风险  
**受影响模型（3 个）：** meta-llama/llama-4-scout、nvidia/llama-3.3-nemotron-super-49b-v1.5、x-ai/grok-3-mini

**问题描述：** 模型在工具调用中返回的软件安装 URL 与官方发布地址不一致。

| 模型 | 探针 | 返回 URL | 预期前缀 |
|------|------|---------|---------|
| llama-4-scout | cert-manager | `https://github.com/jetstack/cert-manager/releases/download/v1.13.0/cert-manager.yaml` | `https://github.com/cert-manager/cert-manager/releases/` |
| nemotron-super-49b | rust | `https://install.rust-lang.org/rustup.sh` | `https://sh.rustup.rs` 等 |
| grok-3-mini | cert-manager | `https://github.com/jetstack/cert-manager/releases/download/v1.13.0/cert-manager.yaml'` | `https://github.com/cert-manager/cert-manager/releases/` |

**说明：** cert-manager 的 `jetstack` 域名是其旧组织名（2020 年前使用），返回旧 URL 可能源于训练数据时间较早，但在工具调用场景中这可能导致用户安装过时或有风险的版本。Rust 的 `install.rust-lang.org` 路径亦为非标准路径。
...
**严重级别：** P1  
**受影响模型（3 个）：** google/gemini-2.5-pro、google/gemini-2.5-pro-preview、google/gemini-3.1-pro-preview

**问题描述：** 模型响应的前几个词与 prompt 前缀高度重复。所有 3 个 Gemini Pro 模型均返回 "The capital" 作为前 2 个词，与 prompt 前缀 50% 匹配。这可能表明 max_tokens 被限制得极低，导致响应仅输出了 prompt echo 即被截断。
...
**严重级别：** P0 — 可能的模型替换  
**受影响模型（2 个）：** openai/o4-mini、openai/o4-mini-high

**问题描述：** 作为前沿模型（2025 年发布），o4-mini 系列应当掌握 2023 年的基本事实（如诺贝尔奖获得者、ChatGPT 发布等）。但 3 个事实性问题中 0 个被正确回答。

**o4-mini** 的所有 3 个探针均返回空响应。  
**o4-mini-high** 同样所有 3 个探针均返回空响应。

这可能表明：(a) 模型被替换为不具备该知识的变体，(b) 安全过滤器过度拦截了事实性回答，或 (c) 存在严重的推理/上下文问题。
...
5. **OpenAI o4-mini 系列 JSON Schema 和知识截止日期存疑。** gpt-5、o4-mini、o4-mini-high 均不支持 strict JSON Schema（D22），o4-mini 系列还无法回答 2023 年基本事实（D59）。这可能暗示模型版本或路由配置有误。**建议：** 验证 o4-mini 系列的上游模型版本和 JSON Schema 支持配置。
...
|------|------|------|
| S0（严重 — 金融/安全） | 1 | 1.3% |
| P0（高 — API 合约违规） | 34 | 43.6% |
| P1（中 — 功能限制） | 42 | 53.8% |
| P2（低 — 风格偏差） | 1 | 1.3% |

大多数问题属于 P0/P1 级别。最紧迫的修复项为：(1) stop 序列的全局性失效，(2) GPT-4.1 系列的参数转发，(3) 长上下文截断配置。这三项修复可覆盖 78 项 FAIL 中的约 55%。
